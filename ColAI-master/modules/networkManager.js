...
export class NetworkManager {
    ...
    async generateNetworkResponse(network, prompt, attachments = []) {
        ...
        try {
            ...
            // Используем Ollama для всех запросов
            const sessionId = `network-${network}`;
            ...
            await this.ollamaManager.getOrCreateSession(sessionId, {
                ...
                maxTokens: networkSettings.max_tokens,
                topP: networkSettings.top_p,
                contextWindow: 4096
            });
            ...
            const ollamaMessages = messages.map(msg => {
                if (msg.role === 'system') {
                    ...
                    if (typeof msg.content === 'string') {
                        ...
                        // Объединяем весь контент в один текст
                        const textParts = msg.content.map(part => {
            ...
            const completion = await this.ollamaManager.sendMessage(sessionId, ollamaMessages, {
                temperature: networkSettings.temperature,
                maxTokens: networkSettings.max_tokens,
                topP: networkSettings.top_p,
                model: this.selectedModel
            ...
            // Provide more meaningful fallback responses based on network role
            const fallbackResponses = {
                'network1': "Let me analyze this topic more systematically. Based on the available information, I need to examine the core components and their logical relationships.",
                'network2': "This opens up interesting creative possibilities. I'm exploring alternative approaches and unconventional solutions that might not be immediately obvious.",
                'network3': "From an implementation perspective, we need to consider the practical constraints and technical requirements for bringing these ideas to reality.",
                'network4': "Looking at this from a data-driven standpoint, I would need to examine patterns, metrics, and empirical evidence to form conclusions.",
                'network5': "This raises important ethical considerations about the societal impact and moral implications we should carefully evaluate.",
                'network6': "From a user experience perspective, we need to focus on how this will actually be used and what the human interaction patterns will be.",
                'network7': "Taking a systems thinking approach, I'm analyzing the interconnections and feedback loops that could emerge from these components.",
                'network8': "I need to challenge some of the assumptions here and identify potential weaknesses or unintended consequences in this approach."
            };
            
            return fallbackResponses[network] || "I'm continuing to analyze this topic and will provide more specific insights as the discussion progresses.";
        }
    }
    ...
    async generateSummary(context, customInstructions = '') {
        ...
        const completion = await this.ollamaManager.sendMessage(sessionId, [
    ...
    async getVoteOnSummary(network, summary) {
        ...
        const sessionId = `network-${network}`;
        const voteSystemPrompt = `${this.networks[network].persona} 
    ...
    async generateFinalOutput(projectName, projectDescription, acceptedSummaries) {
        ...
        try {
            const sessionId = 'final-output';
            const finalSystemPrompt = `You are a discussion synthesizer.
    ...
    async checkOllamaConnection() {
        return await this.ollamaManager.checkConnection();
    }
    ...
    async getAvailableModels() {
        return await this.ollamaManager.getAvailableModels();
    }
}